...
t_basic_flow(Config) when is_list(Config) ->
  ...
  Events = [ {append, 0, 1 , 9 }
           , {append, 1, 10, 39}
           , {append, 2, 40, 49}
           , {switch_append, 3, 1, 10}
           , {switch_append, 4, 1, 10}
           ],
...
t_init_from_existing({init, Config}) ->
  Dir = ?config(dir),
  Idx = init_idx(Dir, [ {append,        0, 1 , 9 }
                      , {append,        1, 10, 19}
                      , {switch_append, 2, 3 , 49}
                      , {append,        3, 50, 59}
                      ]),
...
t_init_from_existing(Config) when is_list(Config) ->
  ...
  Index  = gululog_idx:append(Index1, 4, 1, 9, ts()),
  lists:foreach(fun({LogId, ExpectedLocation}) ->
...
t_read_file_entry_to_locate(Config) when is_list(Config) ->
  ...
  ?assertEqual({0, 10, 19}, gululog_idx:locate(Dir, Idx0, 1)),
  ...
  ?assertEqual({0, 10, 19}, gululog_idx:locate(Dir, Idx1, 1)),
  ?assertEqual({0, 32, 39}, gululog_idx:locate(Dir, Idx1, 3)),
  Idx2 = gululog_idx:delete_from_cache(Idx1, 3),
  ?assertEqual({0, 32, 39}, gululog_idx:locate(Dir, Idx2, 3)),
  ?assertEqual(false, gululog_idx:locate(Dir, Idx0, 4)),
  ok.
...
t_truncated_latest_logid(Config) when is_list(Config) ->
  ...
  ?assertEqual(2, gululog_idx:get_latest_logid(Idx)),
  ok.
...
init_idx(Dir, Idx, [{append, LogId, Pos, EndPos} | Events]) ->
  NewIdx = gululog_idx:append(Idx, LogId, Pos, EndPos, ts()),
  init_idx(Dir, NewIdx, Events);
init_idx(Dir, Idx, [{switch_append, LogId, Pos, EndPos} | Events]) ->
  NewIdx = gululog_idx:switch_append(Dir, Idx, LogId, Pos, EndPos, ts()),
  init_idx(Dir, NewIdx, Events).
